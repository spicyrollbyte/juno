...
class InteractionAgentRuntime:
    ...
    async def execute(self, user_message: str) -> InteractionResult:
        ...
        try:
            ...
            logger.error(f"Interaction agent failed: {exc}", exc_info=True)
            return InteractionResult(
                success=False,
                response="",
                error=str(exc),
    ...
    async def handle_agent_message(self, agent_message: str) -> InteractionResult:
        ...
        try:
            ...
            logger.error(f"Interaction agent (agent message) failed: {exc}", exc_info=True)
            return InteractionResult(
                success=False,
                response="",
                error=str(exc),
    ...
    async def _run_interaction_loop(
        ...
        for iteration in range(self.MAX_TOOL_ITERATIONS):
            ...
            # Stop as soon as a message has been sent to the user — don't loop again
            if user_replied:
                break
        else:
