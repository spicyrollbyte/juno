...
class ExecutionBatchManager:
    ...
    def __init__(self, timeout_seconds: int = 90, user_id: str = "default") -> None:
    ...
    async def _dispatch_to_interaction_agent(self, payload: str) -> None:
        ...
        async def _run() -> None:
            token = set_current_user_id(self.user_id)
            try:
                runtime = InteractionAgentRuntime()
                await runtime.handle_agent_message(payload)
            finally:
                from contextvars import ContextVar
                # reset is best-effort; context is per-task anyway
                pass
