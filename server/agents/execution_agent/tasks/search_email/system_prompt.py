...
from server.utils.timezones import convert_to_user_timezone


def get_system_prompt() -> str:
    """Generate system prompt with today's date for Gmail search assistant."""
    now_user = convert_to_user_timezone(datetime.utcnow())
    ...
    return (
        ...
        f"- Yesterday's date: {yesterday}\n"
        f"- Use these dates as reference for relative time queries (e.g., 'recent', 'today', 'this week')\n"
        f"- IMPORTANT: Gmail's `after:` operator is EXCLUSIVE (after:2026/04/20 means April 21+).\n"
        f"  To find emails received TODAY ({today}), use `after:{yesterday}` not `after:{today}`.\n"
        "\n"
        "## Available Tools:\n"
        ...
        f"   - For \"recent emails from John\": `from:john after:{yesterday}`\n"
        ...
        f"   - For \"today's emails\": `after:{yesterday}` (after is exclusive — use yesterday's date)\n"
