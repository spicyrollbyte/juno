...
from server.anthropic_client import request_chat_completion
...
ERROR_OPENROUTER_NOT_CONFIGURED = "Anthropic API key not configured. Set ANTHROPIC_API_KEY."
...
def _validate_anthropic_config() -> Tuple[Optional[str], Optional[str]]:
    """Validate Anthropic configuration and return (api_key, model) or (None, error)."""
...
async def task_email_search(search_query: str) -> Any:
    ...
    api_key, model_or_error = _validate_anthropic_config()
    if not api_key:
        logger.error(f"[EMAIL_SEARCH] Anthropic not configured: {model_or_error}")
...
async def _perform_search(
    *,
    ...
    # Use LLM-provided max_results or default to 5
...
def _extract_assistant_message(response: Dict[str, Any]) -> Dict[str, Any]:
    """Extract assistant message from Anthropic API response."""
