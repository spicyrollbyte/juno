...
class ExecutionAgentRuntime:
    ...
    # Execute Anthropic API call with system prompt, messages, and optional tool schemas
    ...
    async def _execute_tool(self, tool_name: str, arguments: Dict) -> Tuple[bool, Any]:
        ...
        try:
            if inspect.iscoroutinefunction(tool_func):
                ...
                # Run blocking (Composio) calls in a thread pool so they don't block the event loop
                loop = asyncio.get_running_loop()
                result = await loop.run_in_executor(None, functools.partial(tool_func, **arguments))
            return True, result
        except Exception as e:
