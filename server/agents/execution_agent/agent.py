...
SYSTEM_PROMPT_TEMPLATE = _load_prompt("system_prompt.md") or (
    "You are an execution agent. Agent Name: {agent_name}\nPurpose: {agent_purpose}"
)
RECRUITING_PROMPT_TEMPLATE = _load_prompt("recruiting_prompt.md")
STUDY_PLANNER_PROMPT_TEMPLATE = _load_prompt("study_planner_prompt.md")

_RECRUITING_NAMES = {"recruiting tracker", "recruiting", "recruiter tracker"}
_STUDY_PLANNER_NAMES = {"study planner", "study plan", "exam planner"}


def _pick_template(agent_name: str) -> str:
    name = agent_name.strip().lower()
    if name in _RECRUITING_NAMES and RECRUITING_PROMPT_TEMPLATE:
        return RECRUITING_PROMPT_TEMPLATE
    if name in _STUDY_PLANNER_NAMES and STUDY_PLANNER_PROMPT_TEMPLATE:
        return STUDY_PLANNER_PROMPT_TEMPLATE
...
class ExecutionAgent:
    ...
    def build_system_prompt(self) -> str:
        ...
        template = _pick_template(self.name)
        return template.format(agent_name=self.name, agent_purpose=agent_purpose)
