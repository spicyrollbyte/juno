...
_agent_rosters: dict[str, AgentRoster] = {}
...
def get_agent_roster(user_id: str | None = None) -> AgentRoster:
    """Get the roster for the current (or specified) user."""
    from ...services.user_context import get_current_user_id

    uid = user_id or get_current_user_id()
    if uid == "default":
        return _agent_roster
    if uid not in _agent_rosters:
        path = _DATA_DIR / "users" / _slugify_user(uid) / "roster.json"
        _agent_rosters[uid] = AgentRoster(path)
    return _agent_rosters[uid]
