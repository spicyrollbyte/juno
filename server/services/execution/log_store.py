...
_execution_agent_log_stores: dict[str, ExecutionAgentLogStore] = {}
...
def get_execution_agent_logs(user_id: str | None = None) -> ExecutionAgentLogStore:
    """Get the log store for the current (or specified) user."""
    from ...services.user_context import get_current_user_id

    uid = user_id or get_current_user_id()
    if uid == "default":
        return _execution_agent_logs
    if uid not in _execution_agent_log_stores:
        path = _DATA_DIR / "users" / _slugify_user(uid) / "execution_agents"
        _execution_agent_log_stores[uid] = ExecutionAgentLogStore(path)
    return _execution_agent_log_stores[uid]
