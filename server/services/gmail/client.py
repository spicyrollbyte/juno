...
def _set_active_gmail_user_id(user_id: Optional[str]) -> None:
    ...
    _persist("gmail", sanitized or None)
...
def initiate_connect(payload: GmailConnectPayload, settings: Settings) -> JSONResponse:
    ...
    try:
        ...
        try:
            existing = client.connected_accounts.list(user_ids=[user_id], toolkit_slugs=["GMAIL"], statuses=["ACTIVE"])
            existing_data = getattr(existing, "data", None) or (existing if isinstance(existing, list) else None)
            if existing_data:
                return JSONResponse({"ok": True, "redirect_url": None, "already_connected": True, "user_id": user_id})
        except Exception:
            pass
