...
class EmailTextCleaner:
    ...
    def __init__(self, max_url_length: int = 60, max_body_chars: int = 1500) -> None:
    ...
    def clean_email_content(self, message: Dict[str, Any]) -> str:
        ...
        if html_content:
            text = self.clean_html_email(html_content)
        elif text_content:
            text = self.post_process_text(text_content)
        else:
            return ""

        if self.max_body_chars and len(text) > self.max_body_chars:
