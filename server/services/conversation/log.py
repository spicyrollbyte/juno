...
_CONVERSATION_LOG_PATH = _DATA_DIR / "conversation" / "juno_conversation.log"
...
class ConversationLog:
    ...
    def load_transcript(self, max_entries: int = 30) -> str:
        all_entries = list(self.iter_entries())
        if max_entries and len(all_entries) > max_entries:
            all_entries = all_entries[-max_entries:]
        parts: List[str] = []
        for tag, timestamp, payload in all_entries:
    ...
    def record_reply(self, content: str) -> None:
        timestamp = self._append("juno_reply", content)
        self._working_memory_log.append_entry("juno_reply", content, timestamp)
...
_conversation_logs: Dict[str, "ConversationLog"] = {}
_conversation_log = ConversationLog(_CONVERSATION_LOG_PATH)  # legacy default


def _slugify_user(user_id: str) -> str:
    return "".join(c if c.isalnum() else "_" for c in user_id).strip("_") or "default"


def get_conversation_log(user_id: str | None = None) -> "ConversationLog":
    from ...services.user_context import get_current_user_id

    uid = user_id or get_current_user_id()
    if uid == "default":
        return _conversation_log
    if uid not in _conversation_logs:
        path = _DATA_DIR / "users" / _slugify_user(uid) / "juno_conversation.log"
        _conversation_logs[uid] = ConversationLog(path)
    return _conversation_logs[uid]
