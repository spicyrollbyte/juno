...
async def _call_anthropic(prompt: SummaryPrompt, model: str, api_key: Optional[str], base_url: Optional[str] = None) -> str:
...
async def summarize_conversation() -> bool:
    ...
    summary_text = await _call_anthropic(prompt, settings.summarizer_model, settings.resolved_api_key, settings.llm_base_url)
