"""Google Sheets service via Composio."""
...
def get_active_sheets_user_id() -> Optional[str]:
...
def execute_sheets_tool(
    ...
    *,
    ...
    try:
        ...
        logger.exception("sheets tool execution failed", extra={"tool": tool_name})
...
def initiate_connect(user_id: str, auth_config_id: str) -> Dict[str, Any]:
    ...
    client = _get_composio_client()
    try:
        existing = client.connected_accounts.list(user_ids=[user_id], toolkit_slugs=["GOOGLESHEETS"], statuses=["ACTIVE"])
...
def fetch_status(user_id: str) -> Dict[str, Any]:
    ...
    client = _get_composio_client()
    try:
        accounts = client.connected_accounts.list(user_ids=[user_id], toolkit_slugs=["GOOGLESHEETS"], statuses=["ACTIVE"])
        data = getattr(accounts, "data", None) or (accounts if isinstance(accounts, list) else None)
        connected = bool(data)
        if connected:
            _set_active_sheets_user_id(user_id)
        return {"ok": True, "connected": connected, "user_id": user_id if connected else None}
    except Exception as exc:
        logger.warning("sheets status check failed: %s", exc)
        return {"ok": False, "connected": False, "user_id": None}


def disconnect_account(user_id: str) -> Dict[str, Any]:
    _set_active_sheets_user_id(None)
    return {"ok": True}


__all__ = [
    "execute_sheets_tool",
    "get_active_sheets_user_id",
    "initiate_connect",
    "fetch_status",
    "disconnect_account",
]
